...
#
# This script will fetch the events (max. 10,000) for detectors (max. 1,000)
# the given Org.
#
...
def find_events(detectors_dict):
    ...
    with open("active_detectors.csv", "w", encoding="UTF8") as f:
        ...
        for k, v in track(detectors_dict.items(), "Pulling Detector Events..."):
            ...
            events = requests.get("https://api." + args["realm"] + ".signalfx.com/v2/detector/" + k + "/events?limit=10000", headers=headers).json()
            for z in events:
                ce = ce + 1
            ...
            if ce == 0:
                ...
                table.add_row(
                    "[yellow][link=https://app." + args["realm"] + ".signalfx.com/#/detector-wizard/" + k + "/edit]" + k + "[/link]",
                    "[yellow]" + v,
                    "[bold red]" + str(ce) + "[/bold red]",
                    "[bold orange3]" + str(ct) + "[/bold orange3]",
                )
