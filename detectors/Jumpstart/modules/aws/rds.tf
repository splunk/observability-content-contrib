resource "signalfx_detector" "rds_free_space" {
  name         = "${var.alert_prefix} AWS/RDS Free Space Running Out"
...
resource "signalfx_detector" "rds_DiskQueueDepth_historical_error" {
  name         = "${var.alert_prefix} AWS/RDS rds_DiskQueueDepth for the last 10 minutes where significantly higher than normal, as compared to the mean from the last 2 days"
...
resource "signalfx_detector" "rds_CPU_high_error" {
  name         = "${var.alert_prefix} AWS/RDS CPUUtilization for the last 10 minutes was above 95%"
...
resource "signalfx_detector" "rds_cpu_historical_norm" {
  name         = "${var.alert_prefix} AWS/RDS CPU % has been significantly higher for the past 10 minutes then the historical norm"
...
resource "signalfx_detector" "rds_deadlocks" {
  name         = "${var.alert_prefix} AWS/RDS deadlocks exceeding 0.02 per second"
...
resource "signalfx_detector" "rds_read_latency" {
  name         = "${var.alert_prefix} AWS/RDS Read Latency "
...
resource "signalfx_detector" "rds_write_latency" {
  name         = "${var.alert_prefix} AWS/RDS Write Latency "
...
resource "signalfx_detector" "rds_free_memory" {
  name         = "${var.alert_prefix} AWS/RDS Free Memory Running Out"
