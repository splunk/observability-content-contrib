...
resource "signalfx_detector" "k8s_cluster_cpu_capacity" {
  name         = "${var.alert_prefix} K8S Cluster CPU Capacity approaching limit"
...
resource "signalfx_detector" "k8s_cluster_memory_overcommitted" {
  name         = "${var.alert_prefix} K8S Cluster Memory Overcommitted"
...
resource "signalfx_detector" "k8s_daemonset_ready_vs_scheduled" {
  name         = "${var.alert_prefix} K8S Cluster DaemonSet ready vs scheduled"
...
resource "signalfx_detector" "k8s_deployment_not_at_spec" {
  name         = "${var.alert_prefix} K8S Cluster Deployment is not at spec"
